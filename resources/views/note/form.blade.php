        ...
        <div id="kt_app_toolbar" class="app-toolbar py-3 py-lg-6">
            ...
            <div id="kt_app_toolbar_container" class="d-flex flex-stack w-100">
                ...
                <div class="page-title d-flex flex-column justify-content-center flex-wrap me-3">
                    <!--begin::Title-->
                    <h1 class="page-heading d-flex text-dark fw-bold fs-3 flex-column justify-content-center my-0">Notes Form
                    </h1>
                    <!--end::Title-->
                    <!--begin::Breadcrumb-->
                    ...
                    <div class="col-xl-12">
                        <div class="card ">
                            <div class="card-header align-items-center px-3 min-h-50px">
                                <h3 class=" mb-0">Create Student Report</h3>
                            </div>
                            <div class="card-body p-3 pt-0">
                                ...
                                <div class="row">
                                    ...
                                    <div class="col-md-6 fv-row mb-5">
                                        ...
                                        <input name="title" id="inputTitle" required value="{{ $note->title ?? '' }}"
                                    ...
                                    <div class="col-md-6 fv-row mb-5">
                                        ...
                                        <input readonly="readonly" type="date" name="date" value="{{ date('Y-m-d') }}"
                                            id="date" class="form-control form-control-solid">
                                    </div>
                                    <!--end::Col-->
                                    <!--begin::Col-->
                                    <div class="col-md-6 fv-row mb-5">
                                            ...
                                            <select id="student" name="student_id" class="form-select form-select-solid">
                                                ...
                                                @foreach ($userslist as $user)
                                                    @if ($user->role == 'student')
                                                        <option data-class-id="{{ $user->assigned_class }}"
                                                            value="{{ $student_id = $user->id }}">{{ $user->name }}
                                                        </option>
                                                    @endif
                                                @endforeach
                                            </select>
                                        @else
                                            <select id="student" name="student_id" class="form-select form-select-solid">
                                                <option data-class-id="{{ $note->student->assigned_class ?? '' }}"
                                                    value="{{ $student_id = $note->student_id }}" selected="selected">
                                                    {{ $note->student->name }}</option>
                                            </select>
                                        @endif
                                    </div>
                                    <input type="hidden" id="class_id" name="class_id"
                                        value="{{ $note->student_class_id ?? '' }}" />

                                    <div class="col-md-6 fv-row mb-5">
                                        <label class="form-label">Class Notes</label>
                                        <select id="notes" name="note_id" required
                                    ...
                                    <div class="col-md-6 fv-row mb-5">
                                        ...
                                        <select id="assignments" required name="assignment_id"
                                            class="form-select form-select-solid">
                                    ...
                                    <!--end::Col-->
                                    <!--begin::Col-->
                                    <input hidden name="teacher_id" id="teacher_id" class="form-select form-select-solid"
                                        value="{{ Auth::id() }}">
                                    <!--end::Col-->
                                ...
                                <div class="mb-0 mt-1">
                                    ...
                                    <textarea name="objectives_comments" id="inputObjectivesComments"
                                        class="form-control form-control-solid placeholder-gray-600 fw-bold fs-4 ps-9 pt-7" required rows="6"
                                        name="message">{{ $note->objectives_comments ?? '' }}</textarea>
                                </div>
                                <div class="mb-0 mt-1">
                                    <label class="form-label">Reading Ability</label>
                                    <textarea name="reading_ability_progress" id="inputReadingAbilityProgress"
                                        class="form-control form-control-solid placeholder-gray-600 fw-bold fs-4 ps-9 pt-7" required rows="6"
                                        name="message">{{ $note->reading_ability_progress ?? '' }}</textarea>
                                </div>
                                <div class="mb-0 mt-1">
                                    <label class="form-label">Reading Comprehension</label>
                                    <textarea name="vipers_progress" id="inputVipersProgress"
                                        class="form-control form-control-solid placeholder-gray-600 fw-bold fs-4 ps-9 pt-7" required rows="6"
                                        name="message">{{ $note->vipers_progress ?? '' }}</textarea>
                                </div>
                                <div class="mb-0 mt-1">
                                    ...
                                    <select name="class_objectives" id="inputClassObjectives"
                                        ...
                                        <option value="" disabled
                                            {{ !($note->class_objectives ?? '') ? 'selected' : '' }}>Select Option</option>

                                        <!-- Option for 'Move up' -->
                                        <option value="move_up"
                                            {{ ($note->class_objectives ?? '') == 'move_up' ? 'selected' : '' }}>Move up
                                        </option>

                                        <!-- Option for 'Move down' -->
                                        <option value="move_down"
                                            {{ ($note->class_objectives ?? '') == 'move_down' ? 'selected' : '' }}>Move
                                            down</option>

                                        <!-- Option for 'Maintain' -->
                                        <option value="maintain"
                                            {{ ($note->class_objectives ?? '') == 'maintain' ? 'selected' : '' }}>Maintain
                                        </option>
            ...
            if (studentId) {
                ...
                $.ajax({
                    ...
                    success: function(data) {
                        ...
                        data.forEach(function(note) {
                            $('#notes').append(
                                `<option value="${note.id}" ${note.id == selectedNoteId ? 'selected' : ''}>${note.title}</option>`
                                );
                        });
                    }
                });
                ...
                $.ajax({
                    ...
                    success: function(data) {
                        ...
                        data.forEach(function(assignment) {
                            $('#assignments').append(
                                `<option value="${assignment.id}" ${assignment.id == selectedAssignmentId ? 'selected' : ''}>${assignment.name}</option>`
                                );
                        });
                    }
                });
